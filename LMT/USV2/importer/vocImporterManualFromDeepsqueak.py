...
if __name__ == '__main__':
    ...
    eventTimeLineVoc = EventTimeLine( connection, "Voc DS", loadEvent=False )
    ...
    for number in numberListShort:
        ...
        if int(number) < 300:
            ...
            if extension == "_Stats.xls":
                automaticDf = pd.read_excel(f'{dataFile}')
                for index, row in automaticDf.iterrows():
                    voc = Voc( )
                    startTime = np.round( row["Begin Time (s)"] *1000 ) #convert into milliseconds and round the values
                    endTime = np.round( row["End Time (s)"] *1000 ) #convert into milliseconds and round the values
                    voc.startOffsetMs = startTime
                    voc.endTime = endTime
                    voc.durationMs = np.round( row["Call Length (s)"] *1000 ) #convert into milliseconds and round the values
                    voc.fileName = row["File"]
                    voc.vocNumber = row["ID"]
                    voc.label = row["Label"]
                    voc.accepted = row["Accepted"]
                    voc.score = row["Score"]
                    voc.principalFrequencyKHz = row["Principal Frequency (kHz)"]
                    voc.lowFrequencyKHz = row["Low Freq (kHz)"]
                    voc.highFrequencyKHz = row["High Freq (kHz)"]
                    voc.deltaFrequencyKHz = row["Delta Freq (kHz)"]
                    voc.frequencyStandardDeviationKHz = row["Frequency Standard Deviation (kHz)"]
                    voc.slopeKHz = row["Slope (kHz/s)"]
                    voc.sinuosity = row["Sinuosity"]
                    voc.meanPower = row["Mean Power (dB/Hz)"]
                    voc.tonality = row["Tonality"]
                    voc.peakFrequencyKHz = row["Peak Freq (kHz)"]
